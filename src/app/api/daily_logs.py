...
@router.post("/farms/{farm_id}/logs", response_model=DailyLogResponse, status_code=status.HTTP_201_CREATED)
...
def get_daily_log(
    ...
    log = db.query(DailyLog).join(Farm).filter(
        DailyLog.id == log_id,
...
def get_user_daily_logs(
    ...
    return db.query(DailyLog).join(Farm).filter(
        Farm.user_id == current_user.id
    ).order_by(DailyLog.log_date.desc()).offset(skip).limit(limit).all()