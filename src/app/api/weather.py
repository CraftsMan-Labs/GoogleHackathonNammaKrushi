...
def create_weather_record(
    ...
    db_weather = WeatherHistory(user_id=current_user.id, **weather_data.model_dump())
...
def get_current_weather(
    ...
    db_weather = WeatherHistory(user_id=current_user.id, **weather_data)
...
def get_weather_forecast(
    ...
    if days > 7:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Forecast limited to 7 days"
        )
    ...
    recent_forecast = (
        ...
        .order_by(WeatherHistory.forecast_days_ahead)
        .limit(days)
        .all()
    )
    ...
    for day in range(1, days + 1):
        ...
        forecast_weather = WeatherHistory(
            ...
            rainfall_probability=min(
                80, base_weather["rainfall_probability"] + (day % 4) * 10
            ),
...
def get_weather_history(
    ...
    return (
        ...
        .order_by(WeatherHistory.date.desc())
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def get_weather_record(
    ...
    weather = (
        db.query(WeatherHistory)
        .filter(
            WeatherHistory.id == weather_id, WeatherHistory.user_id == current_user.id
        )
        .first()
    )
